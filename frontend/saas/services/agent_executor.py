...
class CDPConnection:
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            if target_id:
                ...
                result = await self._client.send.Target.attachToTarget(params={"targetId": target_id, "flatten": True})
    ...
    async def get_interactive_elements(self) -> ElementList:
        ...
        def check_disabled_or_readonly(
            ...
            readonly = attrs.get("readonly", "").lower() in ("true", "") or attrs.get("aria-readonly", "").lower() in (
                "true",
                "true",
            )
            # AX properties
        ...
        def walk_doc(doc: dict[str, Any]) -> None:
            ...
            for node in nodes:
                ...
                # 3. Interactive tag
                    ...
                    self._is_interactive_tag(tag_name)
                    or self._is_interactive_ax_role(ax_role)
                    or self._is_interactive_attr_role(role_attr)
                    or any(attr in attrs for attr in ("onclick", "onmousedown", "onmouseup", "onkeydown", "tabindex"))
                    or any(
                        k in (attrs.get("class", "") + " " + attrs.get("id", "")).lower()
